 ...
 * @details dest_mA can be between -51,200,000mA and
 *		51,198,437mA but typically between -5,120mA and
 *		5,119mA depending on rsense_uOhm.
 */
...
/**
 * @brief	Converts an MAX17205 resistance value from a
 *			register in mOhms to a value between 0mOhm and
 *			15,999mOhm.
 */
#if 0
								...
								struct sensor_value *valp)
{
	...
	case SENSOR_CHAN_CURRENT:
		raw = data->current;
		convert_current(config->rsense_mohms, raw, valp);
		break;
...
static int max17205_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	case MAX17205_CHAN_CURRENT_MAX:
		...
		dest = &data->current_max;
		break;
	case MAX17205_CHAN_CURRENT_MIN:
		reg_addr = MAX17205_AD_MAXMINCURR;
		dest = &data->current_min;
		break;
	case SENSOR_CHAN_GAUGE_FULL_CHARGE_CAPACITY:
	...
	case SENSOR_CHAN_GAUGE_NOM_AVAIL_CAPACITY:
		...
		dest = &data->reported_cap;
		break;
	case SENSOR_CHAN_GAUGE_TIME_TO_EMPTY:
		reg_addr = MAX17205_AD_TTE;
		dest = &data->time_to_empty;
		break;
	case SENSOR_CHAN_GAUGE_TIME_TO_FULL:
		reg_addr = MAX17205_AD_TTF;
		dest = &data->time_to_full;
		break;
	case MAX17205_CHAN_AVAILABLE_SOC:
		reg_addr = MAX17205_AD_AVSOC;
		dest = &data->available_soc;
		break;
	case MAX17205_CHAN_PRESENT_SOC:
		reg_addr = MAX17205_AD_VFSOC;
		dest = &data->present_soc;
		break;
	case MAX17205_CHAN_REPORTED_SOC:
		reg_addr = MAX17205_AD_REPSOC;
		dest = &data->reported_soc;
		break;
	case SENSOR_CHAN_GAUGE_CYCLE_COUNT:
		reg_addr = MAX17205_AD_CYCLES;
		dest = &data->cycle_count;
		break;
	case MAX17205_CHAN_LEARN_STAGE:
...
#define MAX17205_INIT(n)													  \
	...
	static const struct max17205_config max17205_config_##n = {				  \
		...
		.cell_bal_thresh_voltage = DT_INST_PROP(n, cell_bal_thresh_voltage),  \
